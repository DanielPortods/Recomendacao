    ...
    def do_PSO(self):
        ...
        if self._check_bounds(lb=self.p_lbound, up=self.p_ubound) \
           and self._check_bounds(lb=self.o_lbound, up=self.o_ubound):
            ...
            to_skip[0] = False
            blue = PSO(func=Dominio().blue_objective_func, n_dim=2, pop=50, max_iter=100,
        ...
        if self._check_bounds(lb=self.g_lb, up=self.g_ub):
            to_skip[1] = False
            green = PSO(func=Dominio().green_objective_func, n_dim=1, pop=50, max_iter=100, \
    ...
    def do_GA(self):
        ...
        if self._check_bounds(lb=self.p_lbound, up=self.p_ubound) \
           and self._check_bounds(lb=self.o_lbound, up=self.o_ubound):
            ...
            to_skip[0] = False
            blue_ga = GA(func=Dominio().blue_objective_func, n_dim=2, size_pop=50, max_iter=100,
                         prob_mut=0.005, lb=[self.p_lbound, self.o_lbound], ub=[self.p_ubound, 
                        self.o_ubound], precision=1e-7)
            
            blue_ga_rec = blue_ga.run()

        if self._check_bounds(lb=self.g_lb, up=self.g_ub):
            to_skip[1] = False
            green_ga = GA(func=Dominio().green_objective_func, n_dim=1, size_pop=50, max_iter=100, \
                          prob_mut=0.01, lb=[self.g_lb], ub=[self.g_ub], precision=1e-7)
